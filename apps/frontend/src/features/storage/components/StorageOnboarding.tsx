...
export function StorageOnboarding({ onSelected, compact = false }: StorageOnboardingProps) {
  ...
  function handleSelect(id: StorageProviderId): void {
    setPending(id);
    selectProvider(id);
    // selectProvider is synchronous but we keep a brief pending state so
    // the click feels acknowledged rather than instantaneous-to-the-point
    // of feeling unresponsive.
    window.setTimeout(() => {
      setPending(null);
      onSelected?.(id);
    }, 300);
